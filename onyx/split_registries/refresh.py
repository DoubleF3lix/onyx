...
def create_registry(name, url):
    registry_output = []
    registry_output.append("import enum\n\n\n")
    registry_output.append(f"class {name}(enum.Enum):\n")
    for item in requests.get(url).json()["values"]:
        registry_output.append(f"\t{item.replace('minecraft:', '')} = \"{item}\"\n")
...
def create_registry_from_literal(name, location):
    ...
    registry_output.append("import enum\n\n\n")
    registry_output.append(f"class {name}(enum.Enum):\n")
    for item in requests.get("https://raw.githubusercontent.com/Arcensoth/mcdata/master/generated/reports/commands.json").json()["children"][location]["children"]:
