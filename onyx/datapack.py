...
def disable(datapack_name: str):
    ...
    Handler._write(f"datapack disable \"file/{datapack_name}\"")
    ...
    if datapack_pos:
        if datapack_pos.value in {"before", "after"}:
            if not relative_to:
                ...
                Handler._write(f"datapack enable \"file/{datapack_name}\"")
                ...
                Handler._write(f"datapack enable \"file/{datapack_name}\" {datapack_pos.value} \"file/{relative_to}\"")
                return f"datapack enable \"file/{datapack_name}\" {datapack_pos.value} \"file/{relative_to}\""

        Handler._write(f"datapack enable \"file/{datapack_name}\" {datapack_pos.value}")
